...
async function main() {
  ...
  const token = await Token.deploy("WDT", "WDT", 50);
  const timelock = await TimeLock.deploy(minDelay, [await proposer.getAddress()], [await executor.getAddress()], await admin.getAddress());
  let funds = ethers.utils.parseEther('25');
  const treasury = await Treasury.deploy(await executor.getAddress(), { value: funds })
  ...
  //get against, for and abstain votes
  ...
  // Queue the action that has been proposed (if state is success)
  ...
  // Execute the action that has been proposed (if state is success)
